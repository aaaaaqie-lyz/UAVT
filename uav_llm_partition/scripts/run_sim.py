...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Comma-separated schedulers to run under the same seed/topology (e.g. heuristic,marl).",
    )
    ...
    def _run_once(scheduler_type: str) -> None:
        sim = Simulator(
            num_uav=num_uav,
            num_layers=2,
            num_heads=4,
        ...
        lines = [
            f"Simulation summary ({scheduler_type}): {{",
    ...
    if args.compare:
        schedulers = [s.strip().lower() for s in args.compare.split(",") if s.strip()]
        for sched in schedulers:
            _run_once(sched)
    else:
        _run_once(args.scheduler)
