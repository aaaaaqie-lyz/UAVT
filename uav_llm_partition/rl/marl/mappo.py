    ...
    def learn(
        ...
        for _ in range(self.cfg.ppo_epochs):
            ...
            for start in range(0, len(indices), batch_size):
                ...
                for agent_id, actor in enumerate(self.actors):
                    actor_policy_loss, actor_entropy = actor.update_ppo(
                ...
                agent_count = max(self.cfg.num_agents, 1)
                total_policy_loss += policy_loss / (steps_count * agent_count)
                total_value_loss += value_loss / steps_count
                total_entropy += entropy_acc / (steps_count * agent_count)
                if log_prob_count:
                    total_kl += 0.0
