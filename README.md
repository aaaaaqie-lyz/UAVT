...
- **RL scheduling option:** A lightweight REINFORCE-style agent and environment can be enabled via `scheduler_type="rl"` for
  block-level device selection experiments, and a multi-agent variant via `scheduler_type="marl"` for per-UAV bidding.
...
- `uav_llm_partition/controller`: Weight manager, Lyapunov queue, heuristic scheduler, RL/MARL scheduler hooks, and state collector.
- `uav_llm_partition/rl`: Lightweight RL agent, environment, and training utilities for block-to-device assignment, plus MARL
  primitives under `rl/marl`.
